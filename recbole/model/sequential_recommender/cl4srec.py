...
class CL4SRec(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        # ablation
        self.vanilla = "vanilla" in config
        self.only_crop = "only_crop" in config
        self.only_mask = "only_mask" in config
        self.only_reorder = "only_reorder" in config
        self.only_insert = "only_insert" in config
        self.only_delete = "only_delete" in config
        self.only_replace = "only_replace" in config
        self.only_subset_split = "only_subset_split" in config
        self.all_enabled = "all_enabled" in config
        assert (
            sum(
                [
                    self.vanilla,
    ...
    def augment(self, item_seq, item_seq_len):
        ...
        return (
            torch.stack(aug_seq1),
            torch.stack(aug_len1),
            torch.stack(aug_seq2),
            torch.stack(aug_len2),
        )
    ...
    def calculate_loss(self, interaction):
        ...
        aug_item_seq1, aug_len1, aug_item_seq2, aug_len2 = (
            interaction["aug1"],
            interaction["aug_len1"],
            interaction["aug2"],
            interaction["aug_len2"],
        )
    ...
    def cl4srec_aug(self, cur_data):
        def item_crop(seq, length, eta=0.6):
            ...
            if crop_begin + num_left < seq.shape[0]:
                croped_item_seq[:num_left] = seq[
                    crop_begin : crop_begin + num_left
                ].cpu()
            else:
                croped_item_seq[:num_left] = seq[crop_begin:].cpu()
            return torch.tensor(croped_item_seq, dtype=torch.long).to(
                self.device
            ), torch.tensor(num_left, dtype=torch.long).to(self.device)
        ...
        def item_insert(seq, length, nums=1):
            if length == self.max_seq_length:
                return seq, length

            insert_pos = random.randint(0, length)
            insert_item = random.randint(
                1, self.dataset.item_num
            )  # token 0 has been used for semantic masking
            inserted_item_seq = np.zeros(seq.shape[0])
            inserted_item_seq[:insert_pos] = seq[:insert_pos].cpu()
            inserted_item_seq[insert_pos] = insert_item
            inserted_item_seq[insert_pos + 1 :] = seq[insert_pos:-1].cpu()
            return (
                torch.tensor(inserted_item_seq, dtype=torch.long).to(self.device),
                length + 1,
            )

        def item_delete(seq, length, nums=1):
            delete_pos = random.randint(0, length - 1)
            deleted_item_seq = np.zeros(seq.shape[0])
            deleted_item_seq[:delete_pos] = seq[:delete_pos].cpu()
            deleted_item_seq[delete_pos:-1] = seq[delete_pos + 1 :].cpu()
            return (
                torch.tensor(deleted_item_seq, dtype=torch.long).to(self.device),
                length - 1,
            )

        def item_replace(seq, length, nums=1):
            replace_pos = random.randint(0, length - 1)
            replace_item = random.randint(
                1, self.dataset.item_num
            )  # token 0 has been used for semantic masking
            replaced_item_seq = seq.clone().detach()
            replaced_item_seq[replace_pos] = replace_item
            return replaced_item_seq, length

        def item_subset_split(seq, length, p=0.25):
            ret = np.zeros(seq.shape[0])
            dropped_item_seq = seq.cpu().numpy()[:length]

            while True:
                mask = np.random.rand(length) > p
                if np.any(mask):
                    break
            dropped_item_seq = dropped_item_seq[mask]

            new_length = dropped_item_seq.shape[0]
            ret[:new_length] = dropped_item_seq

            return torch.tensor(ret, dtype=torch.long).to(self.device), torch.tensor(
                new_length, dtype=torch.long
            ).to(self.device)

        seqs = cur_data[self.ITEM_SEQ]
        lengths = cur_data[self.ITEM_SEQ_LEN]
        ...
        for seq, length in zip(seqs, lengths):
            if length > 1:
                if self.only_crop:
                    ...
                    switch = [1, 1]
                elif self.only_reorder:
                    switch = [2, 2]
                elif self.only_insert:
                    switch = [3, 3]
                elif self.only_delete:
                    switch = [4, 4]
                elif self.only_replace:
                    switch = [5, 5]
                elif self.only_subset_split:
                    switch = [6, 6]
                elif self.vanilla:
                    switch = random.sample(range(3), k=2)
                elif self.all_enabled:
                    switch = random.sample(range(7), k=2)
                else:
                    switch = random.sample(range(3), k=2)  # default settings
            ...
            if switch[0] == 0:
                ...
                aug_seq, aug_len = item_insert(seq, length)
            elif switch[0] == 4:
                aug_seq, aug_len = item_delete(seq, length)
            elif switch[0] == 5:
                aug_seq, aug_len = item_replace(seq, length)
            elif switch[0] == 6:
                aug_seq, aug_len = item_subset_split(seq, length)
