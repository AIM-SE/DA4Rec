...
class FilterLayer(nn.Module):
    def __init__(
        self, max_seq_length, hidden_size, hidden_dropout_prob, layer_norm_eps
    ):
        super(FilterLayer, self).__init__()

        self.complex_weight = nn.Parameter(
            torch.randn(1, max_seq_length // 2 + 1, hidden_size, 2, dtype=torch.float32)
            * 0.02
        )  # todo：why？
        self.out_dropout = nn.Dropout(hidden_dropout_prob)
        self.LayerNorm = nn.LayerNorm(hidden_size, eps=layer_norm_eps)
...
class FMLPLayer(nn.Module):
    ...
    def __init__(
        ...
        self.filter_layer = FilterLayer(
            max_seq_length, hidden_size, hidden_dropout_prob, layer_norm_eps
        )
        self.feed_forward = FeedForward(
            hidden_size,
            intermediate_size,
