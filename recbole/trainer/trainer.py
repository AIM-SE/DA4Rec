...
class Trainer(AbstractTrainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        total_loss = None
        total_batch_num = len(train_data)
        iter_data = (
            tqdm(
                train_data,
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            if batch_idx % 4 == 3 or batch_idx == total_batch_num - 1:
    ...
    def fit(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            train_epoch_cost.append(training_end_time - training_start_time)
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                valid_end_time = time()
                valid_epoch_cost.append(valid_end_time - valid_start_time)
                valid_score_output = (
        ...
        return (
            ...
            statistics.mean(train_epoch_cost),
            statistics.mean(valid_epoch_cost),
        )
...
class ICLRecTrainer(Trainer):
    ...
    def _prepare_cluster(self, cluster_dataloader):
        # print("Preparing Clustering:")
        self.model.eval()
        kmeans_training_data = []
        for batch_idx, interaction in enumerate(cluster_dataloader):
            interaction = interaction.to(self.device)
            sequence_output = self.model.get_seq_output(interaction)
            # average sum
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        if (
            self.args.contrast_type in ["IntentCL", "Hybrid"]
            and epoch_idx >= self.args.warm_up_epoches
        ):
            self._prepare_cluster(train_data)
