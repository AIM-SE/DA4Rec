...
class SequentialDataset(Dataset):
    ...
    def data_augmentation(self):
        ...
        ``u1, <i1, i2, i3> | i4``
        """
        self.logger.info("data_augmentation")
    ...
    def build(self):
        ...
        # Check if there's any evaluation task
        if "max_pop" in self.config and "min_pop" in self.config:
            ...
            all_train_item_list = train_dataset[:][
                "item_id_list"
            ]  # train_dataset[:]['item_id_list']
            all_train_item_id = train_dataset[:]["item_id"]

            pos_seq_dict = {}
            pos_target_dict = {}
            for idx, (item_list, item_id) in enumerate(
                zip(all_train_item_list, all_train_item_id)
            ):
                for item in item_list.tolist():
                    item_int = int(item)
                    if item_int == 0:
                        continue
                    if item_int not in pos_seq_dict:
                        pos_seq_dict[item_int] = []
                    pos_seq_dict[item_int].append(idx)

                item_int = int(item_id)
                if item_int not in pos_target_dict:
                    pos_target_dict[item_int] = []
                pos_target_dict[item_int].append(idx)

            if self.config.partitions:
                import pickle

                partition_path = os.path.join(
                    self.config.partitions, self.config.partition_dataset
                )
                partition_filepath = os.path.join(
                    partition_path,
                    f"{self.config.partition_dataset}-{min_pop}-{max_pop}",
                )
                with open(partition_filepath, "rb") as f:
                    partition_list = pickle.load(f)

                selected_idx = partition_list[0]
                selected_item = partition_list[1]

                return [datasets[0], datasets[1], self.copy(test_dataset[selected_idx])]

            for idx, target_item in enumerate(test_dataset[:]["item_id"]):
                target_item = int(target_item)
                cnt = 0
                if target_item in pos_seq_dict:
                    cnt += len(pos_seq_dict[target_item])
                if target_item in pos_target_dict:
                    cnt += len(pos_target_dict[target_item])
                if min_pop <= cnt < max_pop:
                    selected_idx.append(idx)
                    selected_item.append(target_item)

            filtered_dataset = self.copy(test_dataset[selected_idx])
            return [datasets[0], datasets[1], filtered_dataset]
        elif "max_seq" in self.config and "min_seq" in self.config:
