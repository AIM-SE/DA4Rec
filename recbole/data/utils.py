...
def create_dataset(config):
    ...
    if hasattr(dataset_module, config["model"] + "Dataset"):
        ...
        if config["model"] == "DuoRec" and config["SSL_AUG"] == "DuoRec":
            dataset_class = getattr(dataset_module, "DuoRecSequentialDataset")
...
def get_aug_dataloader(config, phase, on_the_fly):
    if phase == "train" and on_the_fly:
        return AugmentedDataLoader
    else:
        return get_dataloader(config, phase)
